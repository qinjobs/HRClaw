...
def _js_time_formatter_helpers(function_name: str = "fmtTime") -> str:
    return f"""
    function pad(value) {{
      return String(value).padStart(2, "0");
    }}

    function parseHrclawTime(value) {{
    ...
    function {function_name}(value) {{
      const date = parseHrclawTime(value);
...
</html>""".replace("__USERNAME__", username).replace("__FMT_TIME_HELPERS__", _js_time_formatter_helpers())
